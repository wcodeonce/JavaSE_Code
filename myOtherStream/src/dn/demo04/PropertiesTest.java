...
/*
    需求:
        请写程序实现猜数字小游戏只能玩三次，如果还想玩，提示：游戏试玩已结束，想玩请充值(www.itcast.cn)
    思路:
        1.写一个游戏类，里面有一个猜数字的小游戏
        2.写一个测试类，测试类中有main方法，main()方法中按照下面步骤完成
            A：从文件中读取数据到 Properties 集合，用 load() 方法实现
                文件中以及存在：game.txt
                里面有一个数据值： count = 0
            B：通过 Properties 集合获取到玩游戏的次数
            C：判断次数是否达到3次了
                如果到了，给出提示：游戏试玩结束，想玩请充值(www.itcast.cn)
                如果不到3次：
                    玩游戏
                    次数+1，重新写回文件，用 Properties 的 store() 方法实现
 */
